...
[VRTestLevel]  ← VR 전용, 유일한 진입점
  ...
  Screen Space UI (운영자 데스크탑):
    └─ WBP_VTC_OperatorMonitor (OperatorController가 자동 생성)
         ├─ Combo_ProfileSelect + [Apply] 버튼 → 프로파일 즉시 적용
         ├─ Btn_CaptureHipPos → Waist 위치를 VehicleHipPosition으로 캡처
         ├─ 실시간 거리 목록 (30Hz)
         └─ CB_TrackerMeshVisible → Tracker 메시 표시/숨김 토글
```
...
ApplyGameInstanceConfig()
    ...
    └─ Widget.UpdateSubjectInfo / UpdatePresetInfo 갱신

[Set Hip Here 버튼]
    → 피실험자가 차량 시트에 앉은 상태에서 클릭
    → 현재 Waist 트래커 위치 → VehicleHipPosition 저장
    → 현재 프로파일 JSON에 즉시 반영
```
...
[VRTestLevel]
  ...
  WBP_VTC_OperatorMonitor     (VTC_OperatorMonitorWidget 기반)
...
| 프로퍼티 | 값 |
|---------|-----|
| Operator Monitor Widget Class | `WBP_VTC_OperatorMonitor` |
...
> - **VR 피실험자**: WBP_VTC_StatusWidget (3D WorldSpace, StatusActor에 배치) — 상태/키 안내
> - **운영자 데스크탑**: WBP_VTC_OperatorMonitor (Screen Space) — 운영자 모니터 전용
...
| ComboBoxString | `Combo_ProfileSelect` | 프로파일 목록 드롭다운 |
| Button | `Btn_ApplyProfile` | 선택한 프로파일 즉시 적용 버튼 |
| CheckBox | `CB_TrackerMeshVisible` | Tracker 3D 메시 표시/숨김 실시간 토글 |
| Button | `Btn_CaptureHipPos` | 현재 Waist 위치를 VehicleHipPosition으로 캡처 |
| TextBlock | `Txt_HipCapture` | 캡처 결과 메시지 표시 |
...
[Canvas Panel]
  └─ [Vertical Box] — 전체 레이아웃, 왼쪽 상단 고정
       ...
       │    ├─ Button Btn_CaptureHipPos            "Set Hip Here"
       │    └─ TextBlock Txt_HipCapture            (캡처 결과 표시)
       │
       ├─ Txt_State             (폰트 굵게, 크게)
...
### BP_VTC_OperatorController에 연결

1. `BP_VTC_OperatorController` 열기
2. **Class Defaults → VTC|Operator|Monitor** 카테고리
3. `Operator Monitor Widget Class` → `WBP_VTC_OperatorMonitor` 선택

BeginPlay에서 자동으로 생성되어 뷰포트에 표시됩니다.
...
3. VR 진입 후: OperatorMonitor에서 프로파일 선택 → **[Apply]** 또는 **P키** 로 설정 적용
