...
### 3-레벨 구조 (VR 레벨 / Sim 레벨 분리)

> **핵심 설계 원칙:** VR 레벨과 Simulation 레벨은 **GameMode, PlayerController가 다르므로 반드시 별도 레벨**로 분리합니다.
> Level 1(Setup)은 선택적 — INI에서 자동 로드하므로 VR/Sim 레벨을 직접 실행해도 됩니다.

```
[Level 1 — VTC_SetupLevel]  ← 선택적 (INI 자동 로드로 생략 가능)
  ...
  WBP_SetupWidget (화면에 표시)
    ...
    └─ [Start Session] 버튼
          → GameInstance.SessionConfig 저장 + INI 저장
          → RunMode=VR  → OpenLevel("VTC_VRTestLevel")
          → RunMode=Sim → OpenLevel("VTC_SimTestLevel")
...
[Level 1 경유 시]
SetupWidget 입력 → [Start Session]
    └─ GI.SessionConfig 업데이트 + SaveConfigToINI()
    └─ OpenTestLevel() → RunMode 기반 분기
         ├─ RunMode=VR  → VTC_VRTestLevel
         └─ RunMode=Sim → VTC_SimTestLevel
...
OperatorController::BeginPlay() → ApplyGameInstanceConfig()
    ...
    └─ VehicleHipPosition → ReferencePoint 런타임 스폰 (시안색 위치 마커)
...
- **게임 시작 시 자동 로드** (`GameInstance::Init()`에서 처리 — Level 1 없이도 동작)
- Level 1 `NativeConstruct()`에서도 LoadConfigFromINI() 호출 (중복 로드 무해)
- **저장 항목:** RunMode, MountOffset 5개, VehicleHipPosition, ShowCollisionSpheres, ShowTrackerMesh, **WarningThreshold_cm, CollisionThreshold_cm** ← 임계값도 저장됨
- **저장 안 함:** SubjectID, Height (매 세션마다 새로 입력)
...
| 프로퍼티 | 값 |
|---------|-----|
| VR Test Level Name | `VTC_VRTestLevel` ← VR 전용 레벨 파일 이름과 일치시킬 것 |
| Sim Test Level Name | `VTC_SimTestLevel` ← Simulation 전용 레벨 파일 이름과 일치시킬 것 |
| Setup Level Name | `VTC_SetupLevel` ← Level 1 레벨 파일 이름과 일치시킬 것 |

> **자동 분기:** [Start Session] → RunMode=VR이면 VRTestLevelName으로, RunMode=Sim이면 SimTestLevelName으로 자동 이동합니다.
> **INI 자동 로드:** 게임 시작 시 `GameInstance::Init()`에서 VTCSettings.ini를 자동으로 로드합니다. Level 1을 방문하지 않아도 이전 설정이 복원됩니다.
...
1. Content Browser → Level → 이름: `VTC_VRTestLevel` ← `BP_VTC_GameInstance.VRTestLevelName`과 일치시킬 것
...
1. Content Browser → Level → 이름: `VTC_SimTestLevel` ← `BP_VTC_GameInstance.SimTestLevelName`과 일치시킬 것
...
5. **Hip Position 자동 스냅:** Level 1에서 Vehicle Hip Position을 설정하면 VR 레벨 시작 시 Waist가 해당 위치로 자동 이동됩니다.
