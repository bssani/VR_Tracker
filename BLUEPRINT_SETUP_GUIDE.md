...
Level 2 로드 → OperatorController::BeginPlay → ApplyGameInstanceConfig()
    ...
    └─ VehicleHipPosition → AVTC_ReferencePoint 런타임 스폰 (모니터 전용 마커)
         └─ bMonitorOnly=true → Waist 거리 측정 (Warning/Collision 미발생), 시안색
              (OperatorMonitorWidget + 디버그 라인에 실시간 거리 표시)
```
...
> - 숫자키 1/2/3/4 세션 제어 + GameInstance 설정 적용 → OperatorController(부모)가 처리
...
3. WarningFeedback이 BeginPlay에서 **자동으로 PostProcessVolume을 탐색**합니다
   - 수동 연결 불필요! 레벨에 PostProcessVolume만 배치하면 됩니다
...
2. WarningFeedback이 BeginPlay에서 **자동으로 탐색**합니다 (수동 연결 불필요)
...
- 1 → 캘리브레이션 / 2 → 테스트 직접 시작 / 3 → CSV 내보내기 / 4 → Level 1 복귀
...
| WarningFeedback.PostProcessVolume | `GetAllActorsOfClass(APostProcessVolume)` BeginPlay 자동 탐색 | No — 자동 탐색됨 (수동 지정도 가능) |
...
> **Vehicle_Hip 마커**: 시안색 (0, 0.7, 1) — `bMonitorOnly=true`로 Waist 거리 측정만 수행 (Warning/Collision 없음). 별도 머티리얼 불필요 (동적 머티리얼이 색상을 직접 설정)
...
- PostProcessVolume이 **Infinite Extent(Unbound)**인지 확인
- WarningFeedback이 BeginPlay에서 자동으로 PostProcessVolume을 탐색합니다
- 자동 탐색 실패 시 로그에 `[VTC] WarningFeedback: PostProcessVolume을 찾을 수 없습니다` 메시지 확인
- 레벨에 PostProcessVolume이 최소 1개 배치되어 있는지 확인
...
- Vehicle_Hip은 `bMonitorOnly = true`로 설정된 **모니터 전용 마커** (시안색)
- Waist와의 거리를 측정하고 화면에 표시하지만, Warning/Collision 피드백은 발생하지 않음
