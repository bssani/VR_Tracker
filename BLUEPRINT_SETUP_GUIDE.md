...
[Level 2A — VTC_VRTestLevel]  ← VR 전용
  ...
  BP_VTC_OperatorController
    ├─ BeginPlay → 액터 자동 탐색 + 델리게이트 바인딩만 수행 (설정 적용 없음)
    ├─ P     → INI에서 설정 재로드 + Pawn Hip 위치 스냅 + 전체 적용 + 위젯 업데이트
    ├─ 1     → 캘리브레이션 시작
...
GameInstance::Init()
    └─ LoadConfigFromINI() — Level 1 없이도 이전 설정 자동 복원
         ...
         └─ LastSelectedProfileName 복원 (VTCConfig/LastProfile.txt에서)

[프로파일 시스템 — 신규 ★]
WBP_VTC_ProfileManager (에디터에서 미리 실행 — 레벨 무관)
    ├─ 피실험자 + 차량 조합별 SessionConfig 저장 → Saved/VTCProfiles/<Name>.json
    └─ 예시: "Subject01_SantaFe.json", "Subject02_GrandStarex.json"

[VRTestLevel 에서 프로파일 적용 — 신규 ★]
WBP_VTC_OperatorMonitor → Combo_ProfileSelect 드롭다운에서 선택
    → [Apply] 버튼 클릭
    → GameInstance.ApplyProfileByName() → SessionConfig 교체 + JSON 저장
    → OnProfileApplied 델리게이트 → OperatorController.OnMonitorWidgetProfileApplied()
    → ApplyGameInstanceConfig() → 전체 Actor 즉시 재적용

[설정 위젯 사용 시 (최초 1회 — 기존 워크플로우)]
...
VRGameMode::InitGame() → GI.RunMode = VR 강제  ← INI값보다 우선
    ↓
OperatorController::BeginPlay() → 자동 탐색 + 델리게이트 바인딩만 수행 (설정 적용 없음)
    └─ 아무 이동 없음 — Pawn은 PlayerStart에서 대기

[P키 누를 때]
OperatorController::Input_P() → ApplyGameInstanceConfig()
    ├─ GI.LoadConfigFromINI()  ← INI 재로드
    ├─ TrackerPawn.bSimulationMode = false  (VR 모드)
    ├─ TrackerPawn.SetTrackerMeshVisible(bShowTrackerMesh)
    ├─ TrackerPawn.SnapWaistToWithRetry(VehicleHipPosition)  ← Pawn을 Hip 위치로 스냅 (1회)
    ...
    └─ Widget.UpdateSubjectInfo / UpdatePresetInfo  ← 위젯 업데이트
...
[프로파일 관리 — Utility Editor]
  WBP_VTC_ProfileManager  (VTC_ProfileManagerWidget 기반) ← 사전 설정 전용 (레벨 무관)
...
|----------|------------|---------|
| TextBlock | `Txt_PresetInfo` | 적용된 차종 프리셋 ("Preset: SantaFe") |
| TextBlock | `Txt_HipWaistDistance` | VehicleHip ↔ Waist 실시간 거리 ("Hip↔Waist: 4.2 cm") |
| ComboBoxString | `Combo_ProfileSelect` | **[신규 ★]** 프로파일 목록 드롭다운 (VRTestLevel용) |
| Button | `Btn_ApplyProfile` | **[신규 ★]** 선택한 프로파일 즉시 적용 버튼 |
| CheckBox | `CB_TrackerMeshVisible` | **[신규 ★]** Tracker 3D 메시 표시/숨김 실시간 토글 |

> **프로파일 드롭다운 동작:**
> - `NativeConstruct()`에서 `Saved/VTCProfiles/` 목록을 자동으로 채움
> - `GameInstance.LastSelectedProfileName`으로 마지막 선택 자동 복원
> - [Apply] 클릭 → `GameInstance.ApplyProfileByName()` → `OperatorController.ApplyGameInstanceConfig()` 재실행
>
> **CB_TrackerMeshVisible:**
> - 시작 시 현재 `SessionConfig.bShowTrackerMesh` 값으로 초기화
> - 체크 변경 즉시 `TrackerPawn.SetTrackerMeshVisible()` 적용
...
[Canvas Panel]
  └─ [Vertical Box] — 전체 레이아웃, 왼쪽 상단 고정
       ├─ [Section] 프로파일 선택  ← 신규 ★
       │    ├─ ComboBoxString Combo_ProfileSelect  (Saved/VTCProfiles/*.json 목록 자동)
       │    ├─ Button Btn_ApplyProfile             "Apply Profile"
       │    └─ CheckBox CB_TrackerMeshVisible      "Tracker Mesh 표시"
       │
       ├─ Txt_State             (폰트 굵게, 크게)
       ...
       ├─ Txt_HipWaistDistance  (선택, 시안색 — "Hip↔Waist: 4.2 cm")
...
- **VR 레벨 시작 시 아무것도 하지 않음** — Pawn은 PlayerStart에서 대기
- **P키** → INI에서 설정 재로드 + Pawn을 VehicleHipPosition으로 이동 + 전체 적용
- **1키** → 캘리브레이션 시작 (앉은 자세 유지, 3초 자동 완료)
- **2키** → 캘리브레이션 없이 테스트 직접 시작
- **3키** → CSV 저장 + 게임 종료 (QuitGame)
- **NumPad 1** → Waist 오프셋 그룹 선택 (StatusWidget에 현재 값 표시)
- **NumPad 2** → 양쪽 무릎(LeftKnee/RightKnee) 오프셋 그룹 선택
- **NumPad 3** → 양쪽 발(LeftFoot/RightFoot) 오프셋 그룹 선택
...
**Q: 프로파일 드롭다운이 비어있어요**
- `Saved/VTCProfiles/` 폴더에 `.json` 파일이 없는 경우 — WBP_VTC_ProfileManager에서 먼저 프로파일을 저장하세요
- 저장 경로: `[프로젝트]/Saved/VTCProfiles/<ProfileName>.json`
...
6. VRTestLevel 실행 → OperatorMonitor의 드롭다운에 자동으로 "Subject01_SantaFe" 표시
7. 선택 → [Apply Profile] → 즉시 적용
```
...
> **SetupWidget(Level 1)과의 관계:**
> - SetupWidget과 동일한 필드를 갖지만, **이름 기반으로 복수 저장**이 가능합니다.
> - SetupWidget은 단일 `VTCSettings.json`에 덮어쓰는 방식,
>   ProfileManager는 이름별로 분리된 파일을 관리합니다.
> - 두 시스템은 독립적으로 공존합니다. 병행 사용 가능.
