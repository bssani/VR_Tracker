...
void UVTC_WarningFeedback::BeginPlay() {
  ...
  // Auto-find PostProcessVolume if not manually assigned
  if (!PostProcessVolume) {
    TArray<AActor *> FoundVolumes;
    UGameplayStatics::GetAllActorsOfClass(
        GetWorld(), APostProcessVolume::StaticClass(), FoundVolumes);
    if (FoundVolumes.Num() > 0) {
      PostProcessVolume = FoundVolumes[0];
      UE_LOG(LogTemp, Log,
             TEXT("[VTC] WarningFeedback auto-found PostProcessVolume: %s"),
             *PostProcessVolume->GetName());
    } else {
      UE_LOG(LogTemp, Warning,
             TEXT("[VTC] WarningFeedback: No PostProcessVolume found in the "
                  "level!"));
    }
  }
}
...
void UVTC_WarningFeedback::ApplyWarningState() {
  // 노란색 화면 테두리
  SetPostProcessVignetteColor(FLinearColor(1.0f, 0.8f, 0.0f, 1.0f),
                              WarningVignetteIntensity);
...
void UVTC_WarningFeedback::ApplyCollisionState(FVector CollisionLocation) {
  // 빨간색 화면 테두리
  SetPostProcessVignetteColor(FLinearColor(1.0f, 0.0f, 0.0f, 1.0f),
                              CollisionVignetteIntensity);
...
void UVTC_WarningFeedback::SpawnCollisionFX(FVector Location) {
  if (CollisionImpactFX && GetWorld()) {
    UNiagaraFunctionLibrary::SpawnSystemAtLocation(
        GetWorld(), CollisionImpactFX, Location, FRotator::ZeroRotator,
        FVector::OneVector, true, true, ENCPoolMethod::AutoRelease);
  }
}

void UVTC_WarningFeedback::SetPostProcessVignetteColor(FLinearColor Color,
                                                       float Intensity) {
  if (!PostProcessVolume)
    return;
